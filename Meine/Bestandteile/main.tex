...
\begin{lstlisting}[caption={Hello World in React}]
// index.html:
<!DOCTYPE html>
<html>
<head>
...
When we start the react app, open it in the browser and select inspect we will see the following in the "Elements" tab:
...
On the initial render react will create a component from which it'll then build the DOM the browser then parses and paints on the screen. To show how react determines which part of the DOM it has to update, we'll enhance the previously shown application by a counter functionality:
\begin{lstlisting}
// index.js
