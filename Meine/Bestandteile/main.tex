...
\begin{lstlisting}[label={ex213},caption={Matching Nodes Way 1}]
...
First, the Query resolver is hit and it will search for a defined key that is similar to the name mentioned in the highest level of the query object in \autoref{ex241}, in this case "libraries". In the GraphQL schema under \autoref{ex243} we defined that this query will return an array of Library objects. \\
...
Please note that this feature is only available when \emph{APOC} is installed on this Neo4j instance.
...
SUGGEST RULES THAT COULD BE APPLIED
% suggest somes rules that could be applied
