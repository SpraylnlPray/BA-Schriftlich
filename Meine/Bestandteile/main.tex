...
Despite setting the path for the health check to the one the frontend connects to it would still fail with an error code of \emph{404 Not Found}. Later on it came clear that in order to talk to the server the application uses a POST request. This detail was hidden because Apollo takes care of dispatching the calls.

Then the solution was quickly found: The server now has a GET listener on the path \emph{/healthcheck}. This handler will internally try to fetch nodes from the \emph{/graphql} endpoint the application uses and return the status code of this operation. As the healthchecks now didn't return an error code the task didn't get restarted every 5 minutes and the CPU usage by cluster and service dropped significantly:
\begin{figure}[H]
\centering
