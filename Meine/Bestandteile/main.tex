...
A \emph{collapsable} is a node that can hide (collapse) its children and show (expand) them. A Parent-Child-relationship is established by creating a \emph{Part-Of} link between two nodes, where the collapsable is the parent. There are two types of nodes which have this functionality: Containers and Domains. The button offering this appears in the form on the left when the user selects a collapsable node.
...
\captionof{figure}{Flowchart of \autoref{ex:NeoPointers}}
\label{pic:NeoAlgoExplanation}
\end{figure}

This shows that every relationship has two next pointers. Which one will be used for further traversal, depends on if the source node is start- or end-node in the current relationship. \cite{NeoInternals}

In addition to this, the same pointers exist into the other direction, meaning that there are also two pointers called StartPrevious and EndPrevious. The question for selecting which one will be chosen for further iteration stays the same. \cite{NeoInternals}
...
\section{AWS}
AWS is a cloud platform from Amazon that offers many different services in various geographic regions around the world. This project made use of 3 of them to make the application publicly available. This section will give a short introduction and overview over each of them.
...
\captionof{figure}{AWS usage of this application}
\label{ex:AwsMyApp}
\end{figure}
...
Both, \emph{nodes} and \emph{links} in the application, will however be nodes with properties on them in Neo4j.
